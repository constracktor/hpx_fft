...
then
    ...
    # MPI
    srun -t 1:00:00 --exclusive -n 1 ./build/fftw_mpi_threads 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 1
    for (( j=1; j<$LOOP; j=j+1 ))
    do
        srun -t 1:00:00 --exclusive -n 1 ./build/fftw_mpi_threads 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 0
    done
    for (( i=2**$POW_START; i<=2**$POW_STOP; i=i*2 ))
    do
        for (( j=0; j<$LOOP; j=j+1 ))
        do
            srun -t 1:00:00 --exclusive -n $(($i-$OFFSET)) ./build/fftw_mpi_threads 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 0 
        done
    done
    # HPX
...
then
    ...
    # done
    # # MPI
    # mpirun -n 1 srun -t 1:00:00 --exclusive ./build/fftw_mpi_threads 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 1
    # for (( j=1; j<$LOOP; j=j+1 ))
    # do
    #     mpirun -n 1 srun -t 1:00:00 --exclusive ./build/fftw_mpi_threads 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 0
    # done
    # for (( i=2**$POW_START; i<=2**$POW_STOP; i=i*2 ))
    # do
    #     for (( j=0; j<$LOOP; j=j+1 ))
    #     do
    #         mpirun -n $(($i*$i)) srun -t 1:00:00 --exclusive ./build/fftw_mpi_threads 1 $(($i*$BASE_SIZE)) $(($i*$BASE_SIZE)) $FFTW_PLAN 0 --use-hwthread-cpus
    #     done
    # done
    # # hpx
