...
# MPI only one per node
srun -p buran -N 1 -t 1:00:00 --exclusive -n 1 ./build/fftw_mpi_omp 1 $BASE_SIZE $BASE_SIZE estimate 1
for (( j=1; j<$LOOP; j=j+1 ))
do
    srun -p buran -N 1 -t 1:00:00 --exclusive -n 1 ./build/fftw_mpi_omp 1 $BASE_SIZE $BASE_SIZE estimate 0
done
for (( i=2**$POW_START; i<=2**$POW_STOP; i=i*2 ))
do
    for (( j=0; j<$LOOP; j=j+1 ))
    do
        srun -p buran -N $i -t 1:00:00 --exclusive -n i$ ./build/fftw_mpi_omp 1 $BASE_SIZE $BASE_SIZE estimate 0 
    done
done