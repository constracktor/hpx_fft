...
ss_fftw_mpi_threads_matrix = np.genfromtxt(os.path.abspath('./plot/data/strong_scaling/buran_median/strong_runtimes_fftw_mpi_threads_48.txt'), dtype='float', delimiter=';' , skip_header=1)
...
for i in range (n_entries):
    ss_fftw_mpi_threads_averaged[i,:] = np.median(ss_fftw_mpi_threads_matrix[i*n_loop:(i+1)*n_loop,:],axis=0)

###
# strong scaling data for hpx loop 48
ss_loop_48_matrix = np.genfromtxt(os.path.abspath('./plot/data/strong_scaling/buran_median/strong_runtimes_hpx_loop_48.txt'), dtype='float', delimiter=';' , skip_header=1)
n_entries = int(ss_loop_48_matrix.shape[0]/n_loop)
ss_loop_48_averaged = np.zeros((n_entries, ss_loop_48_matrix.shape[1]))
for i in range (n_entries):
    ss_loop_48_averaged[i,:] = np.median(ss_loop_48_matrix[i*n_loop:(i+1)*n_loop,:],axis=0)
...
ss_task_agas_48_matrix = np.genfromtxt(os.path.abspath('./plot/data/strong_scaling/buran_median/strong_runtimes_hpx_task_agas_48.txt'), dtype='float', delimiter=';' , skip_header=1)
...
for i in range (n_entries):
    ss_task_agas_48_averaged[i,:] = np.median(ss_task_agas_48_matrix[i*n_loop:(i+1)*n_loop,:],axis=0)    

###
# strong scaling data for hpx task agas_48_lci
ss_task_agas_48_lci_matrix = np.genfromtxt(os.path.abspath('./plot/data/strong_scaling/buran_median/strong_runtimes_hpx_task_agas_48_lci.txt'), dtype='float', delimiter=';' , skip_header=1)
n_entries = int(ss_task_agas_48_lci_matrix.shape[0]/n_loop)
ss_task_agas_48_lci_averaged = np.zeros((n_entries, ss_task_agas_48_lci_matrix.shape[1]))
for i in range (n_entries):
    ss_task_agas_48_lci_averaged[i,:] = np.median(ss_task_agas_48_lci_matrix[i*n_loop:(i+1)*n_loop,:],axis=0)  

# print how many percent optimized version is faster
print((ss_loop_48_lci_averaged[:,11] + ss_loop_48_lci_averaged[:,14]) / (ss_loop_48_averaged[:,11] + ss_loop_48_averaged[:,14])  * 100)

print((ss_loop_48_averaged[:,11] + ss_loop_48_averaged[:,14]) / (ss_loop_48_lci_averaged[:,11] + ss_loop_48_lci_averaged[:,14]))
# print speedup of LCI compared to FFTW MPI+pthreads
print( ss_fftw_mpi_threads_averaged[:,6] )
print( ss_task_agas_48_lci_averaged[:,7])
print( ss_fftw_mpi_threads_averaged[:,6] / ss_task_agas_48_lci_averaged[:,7])
...
# # MPI total data
# plt.plot(points, ss_fftw_mpi_total_averaged[:,6], 'v-', c=greyscale[0], linewidth=2, label='FFTW3 with MPI')
# MPI + pthreads data
plt.plot(points, ss_fftw_mpi_threads_averaged[:,6], 'v-', c=colors[13], linewidth=2, label='FFTW3 with MPI+pthreads')
# MPI + OpenMP data
plt.plot(points, ss_fftw_mpi_omp_averaged[:,6], 'v-', c=colors[12], linewidth=2, label='FFTW3 with MPI+OpenMP')
# HPX loop dist data
plt.plot(points, ss_loop_48_averaged[:,7], 's-', c=colors[3], linewidth=2, label='HPX for_loop with MPI')
# HPX loop dist data
plt.plot(points, ss_loop_48_lci_averaged[:,7], 's-', c=colors[2], linewidth=2, label='HPX for_loop with LCI')
# # HPX task agas dist data
# plt.plot(points, ss_task_agas_48_averaged[:,7], 'o-', c=colors[5], linewidth=2, label='HPX future agas with MPI')
# # HPX task agas dist data
# plt.plot(points, ss_task_agas_48_lci_averaged[:,7], 'o-', c=colors[4], linewidth=2, label='HPX future agas with LCI')

# # HPX loop opt dist data
# plt.plot(points, ss_loop_48_opt_averaged[:,7], 's--', c=colors[3], linewidth=2, label='HPX for_loop dist opt')
# # HPX task agas dist data
# plt.plot(points, ss_task_agas_48_opt_averaged[:,7], 'o--', c=colors[5], linewidth=2, label='HPX future agas dist opt')
...
plt.savefig('plot/figures/strong_scaling_buran_median_runtime.pdf', bbox_inches='tight')
