...
void fft_2d_task_scatter(vector_2d& values_vec, const unsigned PLAN_FLAG)
{
    ...
    vector_2d trans_values_prep(num_localities);
    // void futures
    std::vector<hpx::future<void>> r2c_futures(n_x_local);
    std::vector<hpx::future<void>> split_x_futures(n_x_local);
    std::vector<std::vector<hpx::shared_future<void>>> comm_x_futures(n_y_local);
    std::vector<hpx::future<void>> c2c_futures(n_x_local);
    std::vector<hpx::future<void>> split_y_futures(n_y_local);
    std::vector<std::vector<hpx::shared_future<void>>> comm_y_futures(n_x_local);
    // holder for communication futures
    std::vector<hpx::shared_future<vector_1d>> communication_futures(num_localities);
    // FFTW plans
...
//    // hpx::future<std::vector<hpx::future<void>>> future_comm_1 = hpx::when_all(split_x_futures);
//     ////////////////////////////////
//     // Communication in x-direction       
...
//     //     });
    ...
    // hpx::wait_all(communication_futures);
...
// // // comm_x_futures[k][i] = communication_futures[i].then(
// // //         [&](hpx::future<void> r)
// // //         {
// // //             auto a = r.get();
// // //             hpx::async(hpx::annotated_function(&transpose, "transpose_x"), 
// // //                                                  std::cref(a),
// // //                                                  std::ref(trans_values_vec[k]),
// // //                                                  factor_in, factor_out,
// // //                                                  2 * k, 2 * i);
...
//     };
    ...
    // for(std::uint32_t i = 0; i < n_y_local; ++i)
    // {
    //     c2c_futures[i] = hpx::async(hpx::annotated_function(&fft_1d_c2c_inplace, "fft_r2c"),
    //                                 plan_1d_c2c, 
    //                                 std::ref(trans_values_vec[i]));

    //     // split_y_futures[i] = c2c_futures[i].then(
    //     // [&, i, num_localities](hpx::future<void> r)
    //     // {
    //     //     r.get();
    //     //     std::cout << i << std::endl;
    //     //     hpx::async(hpx::annotated_function(&split_vector, "split_x"), 
