...
then
    ...
    #     for (( j=0; j<$LOOP; j=j+1 ))
    #     do
    #         ./build/fftw_mpi_omp $(($i*$i))  $(($i*$BASE_SIZE)) $(($i*$BASE_SIZE)) $FFTW_PLAN 0 --use-hwthread-cpus 
    #     done
    # done
    # # MPI
    # mpirun -n 1 ./build/fftw_mpi_omp 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 1
    ...
    #     for (( j=0; j<$LOOP; j=j+1 ))
    #     do
    #         mpirun -n $(($i*$i)) ./build/fftw_mpi_omp 1 $(($i*$BASE_SIZE)) $(($i*$BASE_SIZE)) $FFTW_PLAN 0 --use-hwthread-cpus
    #     done
    # done
    # # hpx
    # ./build/fftw_hpx 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 1
    # for (( j=1; j<$LOOP; j=j+1 ))
    # do
    #     ./build/fftw_hpx 1 $BASE_SIZE $BASE_SIZE $FFTW_PLAN 0
    # done
    # for (( i=2**$POW_START; i<=2**$POW_STOP; i=i*2 ))
    # do
    #     for (( j=0; j<$LOOP; j=j+1 ))
    #     do
    #         ./build/fftw_hpx $(($i*$i))  $(($i*$BASE_SIZE)) $(($i*$BASE_SIZE)) $FFTW_PLAN 0 
    #     done
    # done
